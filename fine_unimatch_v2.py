...
parser = argparse.ArgumentParser(description='UniMatch V2: Fine-tuning')
parser.add_argument('--config', type=str, required=True)
parser.add_argument('--labeled-id-path', type=str, required=True)
parser.add_argument('--unlabeled-id-path', type=str, required=True)
parser.add_argument('--pretrained-path', type=str, default=None, help='Path to maskfactory/segrcdb pretrained checkpoint')
...
def main():
    ...
    # A. DINOv2 Backbone
    if os.path.exists('dinov2_base.pth'):
        state_dict_dino = torch.load('dinov2_base.pth', map_location='cpu')
        model.backbone.load_state_dict(state_dict_dino)
    
    if cfg['lock_backbone']:
        model.lock_backbone()

    # B. Custom Pretrained Decoder
    if args.pretrained_path and os.path.exists(args.pretrained_path):
        if rank == 0: logger.info(f"Loading pretrained checkpoint from: {args.pretrained_path}")
        ...
        for k, v in pretrained_dict.items():
            ...
            if new_k in model_dict:
                if v.shape == model_dict[new_k].shape:
                    ...
                    pass # Shape mismatch (e.g. head) -> Skip
        
        model.load_state_dict(load_dict, strict=False)
        
        if rank == 0: 
            logger.info(f"Successfully loaded {len(load_dict)} layers from pretrained checkpoint.")
            if len(load_dict) == 0:
                logger.warning("!!! WARNING: No layers loaded! Check key names !!!")

    else:
        if rank == 0: logger.info("No pretrained path provided.")

    # -------------------------------------------------------------------------
    # 3. Optimizer Setup (Default: 2 Groups)
    # -------------------------------------------------------------------------
    optimizer = AdamW(
        [
            # Group 0: Backbone
    ...
    start_epoch = 0 # Force start from 0
    ...
    for epoch in range(start_epoch, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            optimizer.param_groups[0]["lr"] = lr                  # Backbone
            optimizer.param_groups[1]["lr"] = lr * cfg['lr_multi'] # Decoder
            ...
            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info('Iters: {:}, LR: {:.7f}, Total loss: {:.3f}, Loss x: {:.3f}, Loss s: {:.3f}, Mask ratio: '
                            '{:.3f}'.format(i, optimizer.param_groups[1]['lr'], total_loss.avg, total_loss_x.avg, 
