...
from dataset.maskfactory_semi import SemiDataset
from model.semseg.dpt import DPT
from util.utils import count_params, init_log, AverageMeter
from util.dist_helper import setup_distributed

parser = argparse.ArgumentParser(description='UniMatch V2 Pre-training (Binary Class Version)')
...
def main():
    ...
    # nclass is 2 (Background=0, Foreground=1)
    model = DPT(**{**model_configs[cfg['backbone'].split('_')[-1]], 'nclass': 2})
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, (img_s1, img_s2, mask_gt, cutmix_box1, cutmix_box2) in enumerate(trainloader_l):
            ...
            # --- Mask CutMix Logic (2-Class Version) ---
            ...
            target_mask_s1[box1_mask] = 0
            target_mask_s2[box2_mask] = 0

            paste_region_s1 = (mask_gt_flip == 1) & box1_mask
            paste_region_s2 = (mask_gt_flip == 1) & box2_mask

            target_mask_s1[paste_region_s1] = 1 
            target_mask_s2[paste_region_s2] = 1 
            ...
            # Loss S1
            loss_s1_pixelwise = criterion(pred_s1, target_mask_s1)
            valid_mask_s1 = (target_mask_s1 != 255).float()
            loss_s1 = (loss_s1_pixelwise * valid_mask_s1).sum() / (valid_mask_s1.sum() + 1e-6)

            # Loss S2
            loss_s2_pixelwise = criterion(pred_s2, target_mask_s2)
            valid_mask_s2 = (target_mask_s2 != 255).float()
        ...
        if rank == 0:
            ...
            if epoch % 10 == 0 or epoch == cfg['epochs'] - 1:
                torch.save(checkpoint, os.path.join(args.save_path, 'maskfactory_' +str(epoch)+ '.pth'))
